...
We will try to upload the file for <b>3</b> times in case of any error, with 3 seconds and 6 seconds delay between the attempts.
...
` }/>
  <div>
    ...
    By default if not configured differently when generating the presigned URL AWS S3 expects the file to uploaded with the HTTP method `PUT.`
  </div>
